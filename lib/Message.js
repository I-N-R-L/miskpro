...
async function chatting (message, client) {
  try {
    ...
    if ((message.isGroup) && (typeof ezio.config.auto.chat.inbox !== 'true') && ezio.config.auto.chat.inbox == true) {
      await client.sendMessage(message.from, { text }, { quoted: message });
    } else if ((message.from) && (typeof ezio.config.auto.chat.group == 'true') {
      await client.sendMessage(message.from, { text }, { quoted: message });
    }
  } catch (error) {
