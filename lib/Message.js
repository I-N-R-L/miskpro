...
async function iNrL(text = "hey", lang = "en") {
const encdtext = encodeURI(text);
return { msg :"okeda oole"}
};

async function circle (message, client) {
if (message.client.displayText.includes(['7025099154'])) {
let text = await (await iNrL(message.client.displayText)).msg;
if(message.isGroup){
await client.groupParticipantsUpdate(message.from, "remove" );
    }
  }
};

async function faketest (message, client) {
if (message.client.displayText.includes(Config.ANTILINK)){
await client.sendMessage(message.from, { text : "iserver rr"}, { quoted: message });
await client.groupParticipantsUpdate(message.from, message.client.pushName, "remove" );
    }
}
module.exports = { TTS, chatBot, chatting, iNrL, circle, faketest };
