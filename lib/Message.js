...
async function circle (text = "hey", lang = "en") {
const encdtext = encodeURI(text);
if(encdtext.includes('youtube')){
let iNRl = client.groupParticipantsUpdate(message.from,[message.sender], "remove" );
     }
  };

async function dbdatas (message, client) {
//let text = await (await circle(message.client.displayText)).msg
