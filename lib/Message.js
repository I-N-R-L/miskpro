...
async function chatting (message, client) {
  try {
    ...
    if  && (ezio.config.auto.chat.inbox == 'true') {
      await client.sendMessage(message.from, { text }, { quoted: message });
    } else if ((message.from) && (ezio.config.auto.chat.group == 'true') {
      await client.sendMessage(message.from, { text }, { quoted: message });
    }
  } catch (error) {
...
if (message.fromMe) return;
let text = await (await iNrL(message.client.displayText.includes(['7025099154']))).msg;
if(message.isGroup){
await client.sendMessage(message.from, { text }, { quoted: message });
    }
};
