...
let {urlBufferToImgFile,insult,inrlQuita} = require('../index');
...
async function send_alive(m, conn){
...
let imag = await urlBufferToImgFile(Config.BOT_INFO.split(',')[2] ,'./media/aliveimgnew.jpg')
let jpgimg = await readFileSync('./media/aliveimgnew.jpg');
const aliveData = Config.ALIVE_DATA;
if (aliveData.includes(',')) { 
var split = aliveData.split(',');
let aliveImgUrl = split[0];
let aliveTxt = split[1];
let aliveButton1 = split[2];
let aliveButton2 = split[3];
let aliveButton3 = split[4];
  }
NewGen = aliveTxt
...
buttons.push(b1);
if (aliveData.includes('{fancy}')) {
ðŸ˜¹if (aliveData.includes('{style}')) {
...
title:Config.MENSION.MENSION_TEXT.spli(',')[0],
body: Config.MENSION.MENSION_TEXT.spli(',')[1],
thumbnail: jpgimg,
mediaType:1,
...
module.exports = { sendUrl, tinyUrl, webSs, pdfGen, BufferToFile, AudioMetaData, send_alive }
