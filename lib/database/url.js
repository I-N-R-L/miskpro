...
function sendUrl(message, client){
...
return await fs.unlinkSync(download)
 ...
 function tinyUrl(message, client){
	    ...
	    try {
		    ...
		    return await client.sendMessage( message.from, { text : "no data from this url "},{ quoted: message });
	    }
    }

function webSs(message, client){
...
function pdfGen(message, client){
...
module.exports = { sendUrl, tinyUrl, webSs, pdfGen }
