...
if (text.includes(',')) {
         ...
         TextForAud = split[1] 
         CreaterForAud = split[0] 
         imgForAud = split[2]
      }
const songBuffer = fs.readFileSync(audio);
const coverBuffer = imgFile
 
const writer = new ID3Writer(songBuffer);
writer.setFrame('TIT2', CreaterForAud || Config.AUDIO_DATA.split(',')[1] )
      .setFrame('TPE1', [TextForAud,Config.AUDIO_DATA.split(',')[0]])
      .setFrame('TALB', TextForAud || Config.AUDIO_DATA.split(',')[0])
      .setFrame('TYER', 1999)
      .setFrame('APIC', {
          type: 3,
          data: coverBuffer,
