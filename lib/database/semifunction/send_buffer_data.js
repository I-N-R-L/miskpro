...
consolor = consolor.replaceAll('&sender', `@$m.sender.split('@')[0]', mentions: [`${m.sender}`]`);
  }
}
...
consolor = consolor.replaceAll('&num', `${numberr.split('@')[0]},mentions: [`${numberr}`]`);
}
...
module.exports = { remove, unscreen, sendUrl, tinyUrl, webSs, pdfGen, BufferToFile, AudioMetaData, send_alive, send_menu, sendRepeat, toGroup, parse_welcom, parse_exit }
