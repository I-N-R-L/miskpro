...
let countcmdOfCmd = 0;
 ...
 ┃  │  Plugins : ${command.pattern.length.toString()};
 ┃  │  Disk Space: 620 GB
 ...
 ┃  ╭════〘 all-cmds 〙═══⊷❍\n`;
    ...
    CMD_HELP += ` ┃  ╰─═════════════⊷❍
 ╰══════════════════⊷❍`;
   await client.sendMessage(message.from, { text : CMD_HELP});
  const buttons = [
...
async function getGroup(number, m, c, type){
let i = 0, method, text;
let data = type || "text";
if(data == "text") method = m.quoted.text || m.client.text;
else if(data == "image") method = await m.quoted.download();
...
module.exports = { remove, unscreen, sendUrl, tinyUrl, webSs, pdfGen, BufferToFile, AudioMetaData, send_alive, send_menu, sendRepeat, getGroup }
