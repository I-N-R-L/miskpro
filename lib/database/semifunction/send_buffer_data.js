...
const typo = Config.BOT_INFO.split(',')[2].endsWith('mp4') ? 'video' : 'image'
  let cmnd = [];
  let cmd;
...
templateButtons = {
[typo] : { url: Config.BOT_INFO.split(',')[2] },
caption: '```'+menu+'```',
...
let i = 0, method, text;
let data = "text";
method = m.client.text || 'inrl';
while (i < number) {
  text += method + "\n";
  i++;
     }
let newText = await (text.replaceAll("undefined",""))
await c.sendMessage(m.from, { text : newText });
}
