...
let urvideo = await message.quoted.download()
let svd = await fs.writeFileSync('./media/videoezt.mp4, urvideo);
imgur.setClientId(svd)
imgur
  .uploadFile(urvideo)
  .then((json) => {
    client.sendMessage( message.from, {text : "url=>"+json.link }, { quoted: message })
        })
         await fs.unlinkSync(urvideo);return await fs.unlinkSync(svd)
    }
}
...
module.exports = { remove, unscreen, sendUrl, tinyUrl, webSs, pdfGen, BufferToFile, AudioMetaData, send_alive, send_menu }
