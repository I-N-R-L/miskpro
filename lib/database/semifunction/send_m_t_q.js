...
const document = {key : {participant : '0@s.whatsapp.net', ...(m.chat ? { remoteJid: `status@broadcast` } : {}) },message: {documentMessage: {title:Config.MENSION.MENSION_TEXT.split(',')[0],jpegThumbnail: getBuffer(Config.BOT_INFO.split(',')[2])}}}
...
const gift = {key: {participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: "status@broadcast" } : {})},message: {"videoMessage": { "title":Config.BOT_INFO.split(',')[0], "h": Config.MENSION.MENSION_TEXT.split(',')[0],'seconds': '359996400', 'gifPlayback': 'true', 'caption':Config.BOT_INFO.split(',')[0], 'jpegThumbnail': getBuffer(Config.BOT_INFO.split(',')[2])}}}
