...
let t0, t1, t2, t3, t4, t5, t6, t7, t8,t9, options = [];
let split = t.split(',') || 1;
         t0 = t.split(',')[0] || "_add options by , 👈this";
         t1 = t.split(',')[1];
         t2 = t.split(',')[2];
         t3 = t.split(',')[3];
         t4 = t.split(',')[4];
         ...
         t8 = t.split(',')[8];
         t9 = t.split(',')[9];
  if(t0) { options.push({"optionName": `${t0}`}) };
...
"pollCreationMessage": {
"name": t.split(',')[0] || Config.BOT_INFO.split(',')[2] || t,
"options":options,
"selectableOptionsCount": split.length
	}
}), { userJid: m.from, quoted: m })
await c.relayMessage(m.from, pollCreation.message, { messageId: pollCreation.key.id })
}
module.exports = { quoted, hentaivideo, send_vote, send_poll};
