const { isUrl , getBuffer , getRandom, toAudio, toPTT, webp2mp4File } = require('../../');
...
if (!message.quoted) return message.send("replay to an audio");
   ...
   let media = await client.downloadAndSaveMediaMessage(_message)
    ffmpeg(media)
        ...
        await fs.unlinkSync(media)
        return await fs.unlinkSync('./media/slow.mp3');
};
...
module.exports = {  sendPhoto, sendVideo, sendVoice, sendGif, sendBassAudio, sendSlowAudio, sendBlownAudio, sendDeepAudio, sendErrapeAudio, sendFastAudio, sendFatAudio, sendNightcoreAudio, sendReverseAudio, sendSquirrelAudio };
