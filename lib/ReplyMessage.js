...
let users = (await Promise.all(
    ...
    for (const user of participant[0].content.filter(item => item.attrs.error == 403)) {
        ...
        const { subject,desc } = await this.conn.groupMetadata(this.from);
...
key: {
fromMe: false, 
participant: `0@s.whatsapp.net`, ...(this.from ? { remoteJid: "" } : {}) 
},
"message": {
"stickerMessage": {
...
"isAnimated": false
 ...
 this.conn.relayMessage(this.from, { requestPaymentMessage: { Message: { extendedTextMessage: { text: "test", currencyCodeIso4217: 'INR', requestFrom: '0@s.whatsapp.net', expiryTimestamp: 8000, amount: 1, background: null }}}}, {})}
};
