...
let sudok = Config.SUDO.replaceAll(' ','');
a = sudok.split(' ');
a.map((t)=>{
t = t+'@s.whatsapp.net';
CreaterAr.push(t);
...
IsSudo = Config.SUDO.trim()+'@s.whatsapp.net';
    ...
    if (this.message) {
     this.quoted = this.quoted == null ? 'inrl' : this.quoted;
      ...
      if (this.quoted) {
        ...
        this.quoted.isGroup = this.quoted.from == undefined || null ? 'false' : this.quoted.from.endsWith("@g.us");
        this.quoted.isBot = this.quoted.id == undefined || null ? 'false': this.quoted.id.startsWith("BAE5") && this.quoted.id == 16;
        this.quoted.fromMe =
          this.quoted.sender == jidNormalizedUser(conn.user && conn.user?.id);
        this.quoted.text = this.quoted.msg.caption == undefined || null ? this.quoted?.msg : this.quoted.msg.caption;
...
const smsg = (conn, m, options = {}) => {
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.text = this.quoted.msg.caption == undefined || null ? this.quoted?.msg : this.quoted.msg.caption;
