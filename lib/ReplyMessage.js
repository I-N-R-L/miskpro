...
let getFile = (source, filename, referer) => {
...
async send(jid, text, quoted, options) {
      await this.sendPresenceUpdate('composing', jid)
      return this.sendMessage(jid, {
         text: text,
         mentions: parseMention(text),
   ...
   */
   ...
   async sendSticker(jid, path, quoted, options = {}) {
      ...
      let {
         extension
      } = await getFile(buffer)
      const media = tmpdir() + '/' + getRandom(extension)
      const result = tmpdir() + '/' + getRandom('webp')
      if (extension === 'webp') {
