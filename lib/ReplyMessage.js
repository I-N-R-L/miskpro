...
const inrlmdbot = require("./perfix");
...
class WAConnection {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      ...
      pathFile = await writeExif(media, {
        packname: options.packname
          ? options.packname
          : inrlmdbot.config.exif.packname,
        author: options.author ? options.author : inrlmdbot.config.exif.author,
   ...
   * @returns
   */
async sendImageAsSticker = async (jid, path, quoted, options = {}) => {
...
/**
  ...
  async sendErrorMessage(jid, error, key, quoted, btn = [], onReat = true) {
    let butn = {
      buttonId: ".report",
      buttonText: { displayText: "nul" },
      type: 1,
    };
    btn.push(butn);
if(Config.REACT =='on'){
    const imog = await inrlmdbot.reactArry("ERROR");
...
class serialize {
  ...
  _client(conn) {
    ...
    this.client.isCreator = [this.client.botNumber,Config.SUDO]
