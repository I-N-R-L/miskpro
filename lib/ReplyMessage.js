    ...
    _message(conn) {
      if (this.message) {
        ...
        if (this.quoted) {
          ...
          this.quoted.isGroup = this.quoted?.from ? 'false' : this.quoted.from.endsWith("@g.us");
          this.quoted.isBot = this.quoted?.id ? 'false': this.quoted.id.startsWith("BAE5") && this.quoted.id == 16;
          this.quoted.fromMe =
            this.quoted?.sender == jidNormalizedUser(conn.user && conn.user?.id);
          this.quoted.text = this.quoted?.msg?.caption  ? this.quoted?.msg : this.quoted?.msg?.caption;
