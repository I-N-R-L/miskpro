const { isUrl , getBuffer , getRandom } = require('../../main/func');
const { toAudio, toPTT } = require('../../scrapers/converter');
const { webp2mp4File } = require('../../scrapers/uploader');
...
let _message = message.quoted.stickerMessage ;
   ...
   return await client.sendMessage(message.from, { video: { url : webpToMp4.result }, caption: config.exif.cap }, { quoted: message });
}
...
if (!message.quoted) return message.reply('reply to a video');;
   ...
   let media = await client.downloadAndSaveMediaMessage(_message)
   let webpToMp4 = await webp2mp4File(media)
   return await client.sendMessage(message.from, { video: { url : webpToMp4.result }, caption: config.exif.cap, gifPlayback: true },{ quoted: message });  
}
