...
let { runtime,getBuffer } = require('../../main/func');
const {getVar} = require('../../database/variable');
const {listall} = require('../fancy/fancy');
const need = '```'+'add link after command'+'```'
const got = require('got');
const {inrlQuita} = require('../../scrapers/addImg');
const os = require('os');
let { mensionImg } = require('../../count');
...
const { inrl, commands } = require('../../main/prefix');
const fs = require('fs')
...
async function tinyUrl(message, client){
           ...
           if(message.quoted){
           text = message.client.text || message.quoted.text;
          }
	    ...
	    try {
		    ...
		    return await client.sendMessage( message.from, { text:'_tinyurl:-_ '+ JSON.parse(body).result });
	    } catch(e) {
...
async function webSs(message, client){
        if (!message.client.text) return await client.sendMessage(message.from, { text :"_need Url_"},{ quoted: message })
...
return await client.sendMessage(message.from,  { audio: sendAudio, mimetype: "audio/mp4", fileName: `${text}.mp3`,}, { quoted: message });
}
async function send_menu(m, c,isButton=false){
let pstime = new Date().toLocaleDateString("EN", {
                year: "numeric",
                month: "long",
                day: "numeric",
            });
let psnewt = new Date().toLocaleString("LK", {
                timeZone: "Asia/Colombo"
            }).split(" ")[1];
try {
  let data = await getVar();
  let {BOT_INFO,PREFIX,WORKTYPE,FOOTER,INSTAGRAM, PASSWORD} = data.data[0];
...
menu +=`\n┃╏ ◦ ${styletext(cmd.replace(/[^a-zA-Z0-9-+]/g,''),1)}`;
        });
        menu += `\n┃┴╮
┃┬╰────────···
┃╰─────═┅═─────`;
  ...
  const buttons = [
        ...
        { buttonId: `${prefix}list`, buttonText: { displayText: "ʟɪꜱᴛ ᴍᴇɴᴜ"}, type: 2, },
        { buttonId: `${prefix}owner`, buttonText: { displayText: "ᴏᴡɴᴇʀ"}, type: 3, },
      ];
let Buttons = {
text: menu
        };
...
return await c.sendMessage(m.from, Buttons);
 } catch(e){
...
return await c.sendMessage(m.from, responseMsg)
  } catch(e){
...
if(WELCOME_MSG == false || WELCOME_MSG == null || WELCOME_MSG == undefined) return;
...
if(typo==true){
responseMsg = {...responseMsg, caption :consolor || 'bye'}
} else if(typo=="button"){
responseMsg = {...responseMsg, text :consolor || 'bye'}
} else if(typo==false){
responseMsg = { text : consolor || 'bye'}
} else if(typo == "sticker"){
responseMsg = responseMsg;
}
