...
class serialize {
    ...
    _message(conn) {
        if (this.message) {
            ...
            this.mimetype = this.msg.mimetype;
            ...
            if(this.reply_message==null) this.reply_message =new Object();
                this.quoted.mimetype = this.reply_message.mimetype = this.quoted?.msg?.mimetype;
                ...
                this.quoted.msg = this.reply_message.msg = this.quoted?this.quoted[this.quoted?.type]:undefined;
                this.quoted.mentions = this.reply_message.mentions = this.msg?.contextInfo?.mentionedJid||undefined;
                this.quoted.id = this.reply_message.id = this.msg?.contextInfo?.stanzaId||undefined;
                this.quoted.sender = this.reply_message.sender = this.msg?.contextInfo?.participant ? jidNormalizedUser(this.msg?.contextInfo?.participant) : undefined;
                this.quoted.from = this.reply_message.from = this.reply_message.jid = this.quoted.sender;
                this.quoted.number = this.reply_message.number = this.quoted.sender.replace(/[^0-9]/g,'');
                this.quoted.isGroup = this.quoted?.from ? this.quoted?.from?.endsWith("@g.us") : false;
                this.quoted.isBot = this.reply_message.isBot = this.quoted.id ? this.quoted?.id?.startsWith("BAE5") && this.quoted?.id.length == 16 : false;
                this.quoted.fromMe = this.reply_message.fromMe = this.quoted?.sender == jidNormalizedUser(conn.user && conn.user?.id)||false;
                this.quoted.text = this.reply_message.text = this.reply_message.message = (this.quoted?.msg?.caption ? this.quoted?.msg?.caption : this.quoted?.msg) || undefined;
                this.quoted.isEval = this.reply_message.isEval = this.quoted.text?(this.quoted.text.includes('require')||this.quoted.text.includes('return')||this.quoted.text.ncludes('await')):false;
                this.quoted.download = (pathFile) => conn.downloadMediaMessage(this.quoted?.msg, pathFile);
                m.quoted.fakeObj = m.reply_message.fakeObj = proto.WebMessageInfo.fromObject({
                key: {
    ...
    _client(conn, createrS) {
        ...
        this.client.quoted = (Object.keys(this.quoted).length != 0) ? this.quoted : this;
        ...
        this.client.document = document = this.msg.documentMessage? true:false;
