...
class serialize {
    ...
    _key(conn) {
        if (this.key) {
            ...
            this.id = this.key.id;
            this.isBot = this.id == undefined  ? 'false' : this.id.startsWith("BAE5") && this.id.length == 16;
    ...
    _message(conn) {
        if (this.message) {
            this.type = getContentType(this.message)? getContentType(this.message) : getContentType(this.message.viewOnceMessageV2.message)
            ...
            if (this.quoted) {
                this.quoted.type = getContentType(this.quoted)?getContentType(this.quoted) : getContentType(this.quoted.viewOnceMessageV2.message)
    ...
    async fullForward(jid, content, options = {}) {
        ...
        return await conn.relayMessage(jid, mm.message, {
    ...
    let typonIx = getContentType(content)
    if(!typonIx) {
    let V3mmfFrdM
         ...
         let mssggN = message.viewOnceMessageV2.message
         if(!Object.keys(options).length === 0) {
               let sending_media = await generateWAMessageFromContent(jid, content, {
               userJid: this.from
               })
                sending_media[mssggN].mimetype = options.mimetype,
                sending_media[mssggN].jpegThumbnail = options.jpegThumbnail,
                sending_media[mssggN].viewOnce = options.viewOnce,
                sending_media[mssggN].fileName = options.fileName,
                sending_media[mssggN].fileLength = options.fileLength,
                sending_media[mssggN].caption = options.caption,
                sending_media[mssggN].contextInfo  = {
         ...
         return await this.conn.relayMessage(jid,sending_media.message,{messageId: m.key.id })
    }
   ...
   if(typeof content == "object" && content.contacts) {
         ...
         if(options.linkPreview) {
                ...
                content.contacts.contacts[0].contextInfo = { externalAdReply:{...V3mmfFrdR}}
            }
