...
if(content?.viewOnceMessageV2?.message){
  ...
  if(Object.keys(options).length != 0){
         ...
         await this.send(sending_media)
await this.send(frdTypex)
...
await this.send(JSON.stringify(sending_media))
         return await this.conn.relayMessage(jid,sending_media.message,{messageId: this.key.id })
         }
      }
    }
