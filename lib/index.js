...
const { verifyOtp } = require('./database/keys');
const { limit } = require('./database/limit');
const { setTrueId, setTrueKey, getTruekey } = require('./database/sdk');
...
//verify
const { GenrateOtp, isVerified, IsMailed, isValidMailid, sendMail, clearUserFromMail, getVerifiedUsers, verifyUser } = require('./verify');
//decrypt
const {decrypt} = require("./decrypt")
//access
const { remainLimit, giveLimit, resetLimit, getLimit, getLimitV2, UpdateLimit, removeUserLimit } = require('./access');
//spam
const { isFiltered,addFilter,isGroupSparmed,addGroupSparmed,userQuickMsgd,addQuickMsgd,userGroupQuickMsgd,addGroupQuickMsgd,SpamAdd,UserSparmed,giveWarn,isWarned } = require('./spam');
//filter
const { addFilterV2,removeFilter,isFilter,sendFilterMessage,getFilterV2 } = require('./filters');
//media
const {stream2buffer,searchYT,downloadMp3,downloadMp4,GenListMessage,TTS,TRT,getYTInfo} = require('./media');
//Ffmpeg
const { sendPhoto, sendVideo, sendVoice, sendGif, sendBassAudio, sendSlowAudio, sendBlownAudio, sendDeepAudio, sendErrapeAudio, sendFastAudio, sendFatAudio, sendNightcoreAudio, sendReverseAudio, sendSquirrelAudio, sendMp4AsMp3 } = require("./Ffmpeg");
//utils
const {AudioMetaData,remove,unscreen,addSpace,sendUrl,tinyUrl,webSs,pdfGen,BufferToFile,send_menu,send_alive,isValidTimeZone} = require('./utils');
//misc
const { welcome_msg,exit_msg, badWordDetect } = require('./misc');
//scrapers
const {ffmpeg,toAudio,toPTT,toVideo,getJson,isIgUrl,getUrl,isNumber} = require('./scrapers');
//fancy
const {Fancy,Fancylist,FancyRandom} = require('./fancy');
//function thets used
...
google({'query': text}).then(res => {
 ...
 async function weather(message){
	    ...
	    try {
		    ...
		    if (response.status === 200) return await message.client.sendMessage( message.from, { text:'climet of ' +text+ ' is \n\n' +
...
module.exports = {
    ...
    const groupMetadata = await m.conn.groupMetadata(m.from).catch(e => {}),
...
truecaller : function truecaller(response){
try {
const outPut =response, fails = "not found!";
if(!outPut.status) return "_Error, try again!_";
let msg ='```'+`╭─❮ ᴛʀᴜᴇᴄᴀʟʟᴇʀ ❯ ❏
...
return "_Error, try again!_";
} 
},
    isBotAdmin : async function isBotADmin(m){
    if(!m.isGroup) return false;
    const userjid = m.conn.user.id.includes(':')? m.conn.user.id.split(':')[0]:m.conn.user.id.split('@')[0];
    const jid = userjid+'@s.whatsapp.net';
    const groupMetadata = await m.conn.groupMetadata(m.from).catch(e => {}),
    participants = await groupMetadata.participants,
    admins = await participants.filter(v => v.admin !== null).map(v => v.id);
...
verifyOtp,
limit,
setTrueId, setTrueKey, getTruekey,
...
GenrateOtp, isVerified, IsMailed, isValidMailid, sendMail, clearUserFromMail, getVerifiedUsers, verifyUser,
decrypt,
remainLimit, giveLimit, resetLimit, getLimit, getLimitV2, UpdateLimit, removeUserLimit,
isFiltered,addFilter,isGroupSparmed,addGroupSparmed,userQuickMsgd,addQuickMsgd,userGroupQuickMsgd,addGroupQuickMsgd,SpamAdd,UserSparmed,giveWarn,isWarned,
addFilterV2,removeFilter,isFilter,sendFilterMessage,getFilterV2,
stream2buffer,searchYT,downloadMp3,downloadMp4,GenListMessage,TTS,TRT,getYTInfo,
googleIt,
weather,
wikiMedia,
ringTone,
...
AudioMetaData,remove,unscreen,addSpace,sendUrl,tinyUrl,webSs,pdfGen,BufferToFile,send_menu,send_alive,isValidTimeZone,
welcome_msg,exit_msg,badWordDetect,
ffmpeg,toAudio,toPTT,toVideo,getJson,isIgUrl,getUrl,isNumber
}
