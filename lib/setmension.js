...
var IsOwner, IsSudo;
var NewMension = ["7593919575", "7025099154"]
let IsBot = conn.user.id.match(/.{1,10}/g);
...
NewMension.push(IsBot, IsOwner)

let IsMe = m.client.displayText.split('@')[1];
console.log(NewMension, IsMe)

if(NewMension.includes(IsMe)){
console.log(NewMension)
conn.sendMessage(m.from, { audio : audio , mimetype: "audio/mpeg", fileName: `media.mp3`, ptt: true, quoted: m,contextInfo: { externalAdReply:{
