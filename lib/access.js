...
async function giveLimit(user, limitt ="5") {
    let rd = false;
     await limit.find({
        session: decryptedPlainText, user: user
    }).then(async (iscmd) => {
...
async function getLimitV2(user) {
let rd = "";
    await limit.find({
       session: decryptedPlainText, user: user
    }).then(async (iscmd) => {
        if (iscmd[0]) {
                rd += `user : ${iscmd[0].user}`;
                rd += `limit : ${iscmd[0].limit}`;
                rd += `count : ${iscmd[0].count}`;
            } else {
                rd = "no data"
            }
    });
    ...
    await limit.find({
       session: decryptedPlainText, user: user
        ...
        await limit.deleteMany({
                user: user, session: decryptedPlainText
            })
        rd = true;
       } else {
      rd = false;
