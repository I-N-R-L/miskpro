...
const os = require('os');
const {commands} = require('./main/prefix');
const package = require('../package.json');
const {listall} = require("./scrapers/fancy/fancy");
...
async function AudioMetaData(imgFile, audio, message, data){
  let {AUDIO_DATA,STICKER_DATA} = data
...
return await message.client.sendMessage(message.from,  { audio: taggedSongBuffer, mimetype: "audio/mp4", fileName: `${text}.mp3`,}, { quoted: message });
}
...
await imgur
  .uploadFile(urvideo)
  .then((json) => {
    message.client.sendMessage(message.from, {text : json.link }, { quoted: message })
        })
...
if(!text.startsWith('http')) message.reply('need a valid url!');
    ...
    return await message.client.sendMessage(message.from , {document: inrlmx, mimetype: 'application/pdf'}, {quoted: message})
 }
...
let {BOT_INFO,PREFIX,WORKTYPE, TIME_ZONE, WA_GRP} = data;
const info_vars = BOT_INFO.split(/[;,]/);
const img_url = info_vars[2] || "https://i.ibb.co/9H2bHZP/0478bd8b748a.jpg";
...
menu +=`\n┃│❒ ${styletext(cmd.replace(/[^a-zA-Z0-9-+]/g,''),1)}`;
      ...
      menu += '\n╰━━━━━━━━━━━━━◉'
return await m.conn.sendMessage(m.from,{ text: menu,
contextInfo:{
...
try {
const { ALIVE_DATA,BOT_INFO,TIME_ZONE} = data;
let date = new Date().toLocaleDateString("EN", {
                year: "numeric",
                month: "long",
                day: "numeric",
...
return await m.conn.sendMessage(m.from, {text:'_'+`your alive msg have an error:-_\n*${e}*\n_try to type :-_ alive help _to set new alive msg!_`})
  }
}
...
module.exports = {AudioMetaData,remove,unscreen,addSpace,sendUrl,tinyUrl,webSs,pdfGen,BufferToFile,send_menu,send_alive,isValidTimeZone};
